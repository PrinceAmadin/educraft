# Traqly — Master AI Build Prompt (v2)
...
This is Traqly's most powerful differentiator against services like TinyURL and whatsapp link generator. Once a short link is created in most tools, the destination is frozen forever. Traqly gives users full control to update a link's destination at any time — without ever changing the slug, QR code, or any printed/shared material. For when a link is creating from scratch with custom message, it gives the user control to change custom messages without actually changing the link slug, QR Code themselves.
...
7. Build the analytics charts and per-link detail page (including destination change markers on the chart).
...
10. Build smart routing (Feature 20) — add rule management UI and rule evaluation in the redirect engine.
...
13. Build password-protected links (Feature 23) — the `/r/[slug]/auth` page and password verification.
...
18. Polish: empty states, loading states, error states, mobile responsiveness, dark mode.
