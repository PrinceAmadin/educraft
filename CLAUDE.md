...
- [x] Existing-ambassador password setup (Sept 2026) — ambassadors registered before `/apply` collected a login have no `User`. `/login/set-password` (linked from `/login`): they enter email or `EC-A-` ID, a 6-digit code goes ONLY to the email on their `Ambassador` record (same safety rules as client sign-in: identical answer whether or not they exist, hashed single-use code, 10 min, 5 tries, per-ambassador + per-IP limits via `ClientLoginAttempt` kinds `amb-*`), then they choose a password; the `User` (role AMBASSADOR) is created and linked on first setup, or the existing one's password is replaced (forgot password), and they sign in at `/login`. Core in `src/lib/services/ambassador-otp.ts` (`AmbassadorLoginCode` table); only `status: Active` ambassadors with a real email qualify, and an email owned by a non-ambassador user is refused. Live check: all 34 active ambassadors had an email and no login.
