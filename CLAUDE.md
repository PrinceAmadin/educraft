...
- [x] Research pipeline (Report Production System §Sub-System 1) — worker's "Step 1 — Research" panel on `/worker/projects/[id]` (`ResearchPanel`) finds, verifies, and imports academic references before any report writing begins. **Search-first, two-track (Sept 2026 redesign — the first version found ~6 refs because Claude-recalled titles mostly don't exist, ~4% verified on a live test):** Claude (`src/lib/anthropic.ts`) writes academic search queries, run against OpenAlex (`src/lib/openalex.ts` — real indexed works with DOIs, 100 candidates/round); Claude title-recall is only a fallback, verified against CrossRef (`src/lib/crossref.ts` — top 5, fuzzy ≥70% or ≥55% with author+year agreement, author+year-contradicting matches rejected, rate-limited to 3 concurrent with 429 retry). Open-access status decides delivery, not survival: Track A = PDF probed for real PDF bytes via Unpaywall/OpenAlex (`src/lib/unpaywall.ts`) → Drive; Track B = paywalled, kept as reference-only (`Reference.access`), listed with DOI links in a "Paywalled References — Access via your university library" Google Doc in the project's Drive folder. Both tracks import into a per-project Zotero collection (`src/lib/zotero.ts`, tagged by track) and go through Tier 2 (CORE/CLOSELY_RELATED/TANGENTIAL/IRRELEVANT); TANGENTIAL/IRRELEVANT are always dropped, up to 3 replacement rounds if below target, <10 relevant → admin review. "Run research again" (`POST .../research/reset`) wipes a job and its Zotero/Drive output for a clean re-run. Live dry run: 100 candidates → 49 relevant (10 CORE; 12 PDF, 37 paywalled). PDFs upload to Google Drive (`src/lib/google-drive.ts`, OAuth-delegated — service accounts have no quota on personal Drive) under `EduCraft Research/{project_id}/` with a shareable link. Modeled as a resumable step-chained job (`ResearchJob`/`Reference` tables, `src/lib/services/research.ts`) driven by the worker's browser polling `POST /api/worker/projects/[id]/research/step` — required because Vercel Hobby's short function timeout can't hold a multi-minute pipeline in one request; safe to close the tab and resume later. A raw-candidate top-up (bounded, extrapolated from observed CrossRef/Unpaywall attrition) kicks in when a topic's first pass falls short of the target reference count.
