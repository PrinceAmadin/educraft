...
export default function WorkersError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error("[admin/workers]", error); }, [error]);
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl bg-zone px-6 py-16 text-center">
      ...
      <p className="text-sm font-medium text-foreground">Could not load workers</p>
      ...
      <Button variant="outline" size="sm" className="mt-1" onClick={reset}>Try again</Button>
    </div>
  );
}
