...
interface DashboardShellProps {
  ...
  defaultRole: NavRole;
  /** Every dashboard this login can open (more than one for a worker who is also an ambassador). */
  portals: NavRole[];
  name: string;
  email: string;
  roleLabel: string;
...
/** Chooses the sidebar and nav from the URL, so one login can move between its worker and ambassador dashboards. */
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <Topbar role={role} portals={portals} name={name} email={email} roleLabel={roleLabel} />
